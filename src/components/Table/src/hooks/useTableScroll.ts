...
export function useTableScroll(
  ...
  function calcModalHeight(tableEl: Element) {
    ...
    if (modalEl) {
      ...
      const modalFooterMarginTop = modalFooterEl ? (modalIsFullscreen ? 0 : parseInt(getComputedStyle(modalFooterEl).marginTop)) : 0

      // 来自于 .ant-modal .ant-modal-body > .scrollbar
      const modalScrollBarHeight = 14

      return (modalTop > modalBottom ? modalTop : modalBottom) + modalFooterHeight + modalFooterMarginTop + modalScrollBarHeight
    }
