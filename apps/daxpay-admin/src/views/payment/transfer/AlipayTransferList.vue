<script lang="ts" setup>
  ...
  import {
    type AlipayTransferOrderQuery,
    type AlipayTransferOrderResult,
    TransferApi,
  ...
  defineOptions({ name: 'AlipayTransferList' });
  ...
  const queryForm = ref<AlipayTransferOrderQuery>({});
  const pageConfig = ref({ currentPage: 1, pageSize: 10, total: 0 });
  const tableData = ref<AlipayTransferOrderResult[]>([]);

  // 详情抽屉
  const drawerVisible = ref(false);
  const drawerLoading = ref(false);
  const detail = ref<AlipayTransferOrderResult>({});
  ...
  async function handleView(row: AlipayTransferOrderResult) {
    drawerVisible.value = true;
    drawerLoading.value = true;
    try {
      const { data } = await TransferApi.alipayGetById(row.id!);
      detail.value = data || {};
    } finally {
  ...
  function handleSync(row: AlipayTransferOrderResult) {
    confirm({
      title: $t('payment.transfer.syncConfirmTitle'),
      content: $t('payment.transfer.syncConfirmContent'),
      onOk() {
        return TransferApi.sync('alipay', row.id!).then(() => {
  ...
  function handleClose(row: AlipayTransferOrderResult) {
    confirm({
      ...
      onOk() {
        return TransferApi.close('alipay', row.id!).then(() => {
